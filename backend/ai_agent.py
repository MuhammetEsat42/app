...
async def run_agent(user: dict, prompt: str, context: dict | None) -> AsyncGenerator[str, None]:
    """Yields SSE lines (data: {...}\n\n). Deducts credits per queued action, hard-stop on empty balance."""
    ...
    if context:
        ...
        opened = context.get("open_script")
        if sel:
    ...
    user_msg = UserMessage(text=prompt + ctx_str)
    ...
    yield sse({"type": "start", "credits": start_credits})
        ...
        while not stopped:
            ...
            for tc in pending:
                ...
                try:
                    ...
                    args = {}
                args = clamp_hard_limits(name, args)
                cost = CREDIT_COST.get(name, 1)

                # Atomic conditional deduction — sole source of truth.
                # Guards concurrent overspend + client-disconnect leak.
                ...
                if res.modified_count == 0:
                    ...
                    yield sse({"type": "hard_stop", "reason": "insufficient_credits",
                               "needed": cost, "remaining": remaining.get("credits", 0)})
                ...
                yield sse({"type": "action", **action})
                chat.add_tool_result(tc.id, json.dumps({"status": "queued", "command_id": cid}))
            user_msg = None
    except Exception as e:
        yield sse({"type": "error", "message": f"AI planner error: {str(e)}"})
        return
    ...
    remaining = (await users_col.find_one({"id": user["id"]}, {"_id": 0, "credits": 1})) or {}
    yield sse({"type": "done", "credits_used": spent,
